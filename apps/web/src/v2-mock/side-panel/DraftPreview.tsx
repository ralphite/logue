...
export function DraftPreview({ candidate, target, onChange, onCitation, onInsert, onUndo }: { candidate: Candidate; target?: TargetSession; onChange: (value: string) => void; onCitation: (sourceId: string) => void; onInsert: () => void; onUndo: () => void }) {
  return (
    <div className="v2-draft-card">
      ...
      <div className="v2-citation-list" aria-label="Draft citations">
        {candidate.citations.map((citation, index) => <button key={`${citation.sourceId}:${index}`} type="button" className="v2-citation-chip" onClick={() => onCitation(citation.sourceId)}><span>{index + 1}</span>{citation.label}</button>)}
      </div>
