...
export function V2SetupRoute({ status, onReady }: { status: ServiceStatus; onReady: () => Promise<void> }) {
  ...
  return <div className="logue-v2 v2-setup-page"><main className="v2-setup-card">
    ...
    {!tested ? <div className="v2-setup-actions"><Button variant="primary" disabled={Boolean(busy) || !connection.base_url.trim() || !connection.model.trim() || !connection.transcription_model.trim()} onClick={() => void test()}>{busy === "test" ? "Checking…" : "Check voice and AI"}</Button><span>Microphone permission is requested only when you first record.</span></div> : <div className="v2-setup-ready" role="status"><Check size={18} /><div><strong>This connection is ready</strong><p><Mic size={14} />Voice configured <span>·</span> <Sparkles size={14} />AI ready</p></div><Button variant="primary" disabled={Boolean(busy)} onClick={() => void save()}>{busy === "save" ? "Saving…" : "Save and open Logue"}</Button></div>}
