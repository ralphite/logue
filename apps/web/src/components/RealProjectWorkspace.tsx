...
export function RealProjectWorkspace({
  ...
  const projectDocuments = useMemo(
    () => documents.filter((document) => document.project === project.name),
    [documents, project.name],
  );
  const projectMaterials = useMemo(
    () =>
      materials.filter((material) => material.projects.includes(project.name)),
  ...
  const activeDocument =
    projectDocuments.find((document) => document.id === activeDocumentId) ??
    projectDocuments[0];
  const [documentTitle, setDocumentTitle] = useState(
    activeDocument?.title ?? `${project.name} brief`,
  );
  ...
  const { size: inspectorWidth, setSize: setInspectorWidth } =
    usePersistentPanelSize({
      storageKey: "logue.panel.project.sources.width.v2",
      defaultSize: 400,
      min: 360,
      max: 640,
    });
    ...
    const timer = window.setTimeout(() => {
      void updateDocument(activeDocument.id, {
        ...
        .then(async (updated) => {
          ...
          if (historyOpen)
            setDocumentRevisions(await getDocumentRevisions(updated.id));
        })
        .catch(() => undefined);
    }, 700);
  ...
  const visibleSourceIds = useMemo(() => {
    ...
    const document = previewRevision ?? activeDocument;
    return document?.context_source_ids?.length
      ? document.context_source_ids
      : (document?.source_ids ?? []);
  }, [activeDocument, answerRun?.sources, previewRevision]);
  const visibleSourceGroups = useMemo(() => {
    ...
    return sourceGroups.filter((group) =>
      group.items.some((item) => ids.has(item.id)),
    );
  ...
  async function restoreRevision() {
    ...
    try {
      const restored = await updateDocument(activeDocument.id, {
        ...
        contextSourceIds: previewRevision.context_source_ids,
        expectedRevision: activeDocument.revision,
      });
      onDocumentsChange(
        documents.map((document) =>
  ...
  async function runProjectRequest() {
    ...
    try {
      ...
      setGenerationError(
        cause instanceof Error
          ? cause.message
          : "Could not generate from this project.",
      );
    } finally {
      setGenerating(false);
    }
  ...
  async function adoptAnswer(action: "copy" | "document") {
    ...
    onDocumentsChange([
      result.document,
      ...documents.filter((document) => document.id !== result.document.id),
    ]);
  ...
  function removeProfileTerm(category: VocabularyCategory, value: string) {
    updateTranscriptionProfile({
      vocabulary: {
        ...transcriptionProfile.vocabulary,
        [category]: transcriptionProfile.vocabulary[category].filter(
          (term) => term !== value,
        ),
      },
    });
  ...
  function addPreferredSpelling() {
    ...
    if (
      !spoken ||
      !preferred ||
      transcriptionProfile.vocabulary.preferred_spellings.some(
    ...
    updateTranscriptionProfile({
      vocabulary: {
        ...transcriptionProfile.vocabulary,
        preferred_spellings: [
          ...transcriptionProfile.vocabulary.preferred_spellings,
          { spoken, preferred },
        ],
      },
    });
  ...
  const bindingRows: Array<{
    key: keyof ProjectSkillBindings;
    label: string;
    fallback?: string;
    accepts: (skill: LogueSkill) => boolean;
  }> = [
    {
      key: "organization",
      label: "Organization",
      fallback: globalDefaults.default_organization_skill,
      accepts: (skill) => skill.task === "organize",
    },
    {
      key: "command",
      label: "Voice Command",
      fallback: globalDefaults.default_extension_skill,
      accepts: (skill) =>
        skill.task === "generate" &&
        skill.output === "insert" &&
    ...
    },
    {
      key: "ask",
      label: "Ask",
      fallback: globalDefaults.default_qa_skill,
      accepts: (skill) => skill.task === "generate" && skill.output === "qa",
    },
    {
      key: "draft",
      label: "Draft",
      fallback: globalDefaults.default_document_skill,
      accepts: (skill) =>
  ...
  async function setProjectMembership(
    group: (typeof contextGroups)[number],
    next: "added" | "removed" | "excluded" | "undo-exclusion",
  ) {
      ...
      await Promise.all(
        group.items.map((material) => {
          ...
          const excludedProjects =
            next === "excluded"
              ? Array.from(
                  new Set([...(material.excludedProjects ?? []), project.name]),
                )
              : (material.excludedProjects ?? []).filter(
                  (name) => name !== project.name,
                );
          const savedOnlyProjects =
            next === "removed" || next === "undo-exclusion"
              ? Array.from(
                  new Set([
                    ...(material.savedOnlyProjects ?? []),
                    project.name,
                  ]),
                )
              : (material.savedOnlyProjects ?? []).filter(
                  (name) => name !== project.name,
                );
          return onUpdateMaterialClassification(
            material.id,
            projects,
            material.tags,
            excludedProjects,
    ...
    <main className="logue-v2 v2-project-main min-h-0 min-w-0 flex-1 overflow-hidden bg-white">
      <header className="v2-project-topbar">
        <div className="v2-breadcrumbs">
          <button
            type="button"
            className="v2-source-excerpt-toggle !mt-0"
            onClick={onBack}
          >
        ...
        <div className="v2-topbar-actions">
          ...
          <Button
            size="sm"
            variant={activeView === "context" ? "primary" : "secondary"}
            onClick={() =>
              setActiveView(activeView === "context" ? "document" : "context")
            }
          >
          ...
          <Button
            ...
            onClick={() =>
              setActiveView(activeView === "settings" ? "document" : "settings")
            }
          >
          ...
          {activeView === "document" && (
            <TooltipProvider>
              <Tooltip
                content={inspectorOpen ? "Close sources" : "Open sources"}
              >
                <IconButton
                  label={inspectorOpen ? "Close sources" : "Open sources"}
                  variant="ghost"
          ...
          {activeView === "settings" ? (
              ...
              <div className="v2-list-axis">
                <div className="v2-page-heading">
                  <div className="v2-page-heading-copy">
                    ...
                    <p>
                      Context used for transcription, classification, Ask, and
                      Draft.
                    </p>
                  </div>
                </div>
                <section className="v2-setting-section">
                  ...
                  <textarea
                    ...
                    placeholder="Goals, decisions, constraints, and working context…"
                  />
                </section>
                <section className="v2-setting-section">
                  ...
                  <div
                    ...
                    role="group"
                    aria-label="Project transcription profile mode"
                  >
                    {(["inherited", "customized", "disabled"] as const).map(
                      (mode) => (
                        <Button
                          key={mode}
                          size="sm"
                          aria-pressed={transcriptionProfile.mode === mode}
                          variant={
                            transcriptionProfile.mode === mode
                              ? "primary"
                              : "secondary"
                          }
                          onClick={() => updateTranscriptionProfile({ mode })}
                        >
                  ...
                  <p className="v2-settings-lead">
                    {transcriptionProfile.mode === "customized"
                      ? "Uses Default personal context plus this Project’s context and vocabulary."
                      : transcriptionProfile.mode === "disabled"
                        ? "Uses only the Default voice profile; Project provenance is still recorded."
                        : "Uses the Default voice profile with this Project’s context."}
                  </p>
                  {transcriptionProfile.mode === "customized" && (
                    <div className="mt-4 space-y-4">
                      <div className="v2-setting-row">
                        <div>
                          ...
                          <p>
                            {skillBindings.transcription
                              ? `Project override · ${skillName(skillBindings.transcription)}`
                              : `Inherits Default · ${skillName(globalDefaults.default_transcription_skill)}`}
                          </p>
                        </div>
                        <select
                          className="v2-input"
                          aria-label="Transcription Skill"
                          value={skillBindings.transcription ?? ""}
                          onChange={(event) =>
                            onSkillBindingsChange({
                              ...skillBindings,
                      ...
                      <div className="v2-setting-row">
                        <div>
                          ...
                          <p>The language Logue should expect most often.</p>
                        </div>
                        <input
                          className="v2-input"
                          value={transcriptionProfile.primary_language}
                          onChange={(event) =>
                            updateTranscriptionProfile({
                      ...
                      <div className="v2-setting-row">
                        <div>
                          ...
                          <p>
                            Optional languages that may appear in the same
                            recording.
                          </p>
                        </div>
                        <input
                          className="v2-input"
                          value={transcriptionProfile.mixed_languages.join(
                            ", ",
                          )}
                          onChange={(event) =>
                            updateTranscriptionProfile({
                      ...
                      <div>
                        <strong className="text-[14px] text-[#3f413c]">
                          Project vocabulary
                        </strong>
                        <div className="mt-2 space-y-2">
                          {vocabularyCategories.map((category) =>
                            transcriptionProfile.vocabulary[category.key]
                              .length ? (
                              <div
                                key={category.key}
                                className="flex flex-wrap items-center gap-2"
                              >
                                <span className="w-20 text-[13px] text-[#8a8b86]">
                                  {category.label}
                                </span>
                                {transcriptionProfile.vocabulary[
                                  category.key
                                  ...
                                  <button
                                    ...
                                    key={value}
                                    onClick={() =>
                                      removeProfileTerm(category.key, value)
                                    }
                                  >
                                    {value} ×
                                  </button>
                        ...
                        >
                          ...
                          <input
                            ...
                            value={profileTerm}
                            onChange={(event) =>
                              setProfileTerm(event.target.value)
                            }
                            onKeyDown={(event) => {
                              if (event.key === "Enter") {
                          ...
                          <Button
                            size="sm"
                            onClick={addProfileTerm}
                            disabled={!profileTerm.trim()}
                          >
                            Add
                          </Button>
                        </div>
                      ...
                      <div>
                        <strong className="text-[14px] text-[#3f413c]">
                          Acronym and preferred spelling
                        </strong>
                        <div className="mt-2 flex flex-wrap gap-2">
                          {transcriptionProfile.vocabulary.preferred_spellings.map(
                            (entry) => (
                              <button
                                type="button"
                                className="v2-membership-pill"
                                ...
                                onClick={() =>
                                  updateTranscriptionProfile({
                                    vocabulary: {
                                      ...transcriptionProfile.vocabulary,
                                      preferred_spellings:
                                        transcriptionProfile.vocabulary.preferred_spellings.filter(
                                          (value) =>
                                            value.spoken !== entry.spoken,
                                        ),
                                    },
                                  })
                                }
                        ...
                        >
                          ...
                          <Button
                            size="sm"
                            onClick={addPreferredSpelling}
                            disabled={
                              !spokenTerm.trim() || !preferredTerm.trim()
                            }
                          >
                            Add
                          </Button>
                      ...
                      <div>
                        ...
                        <textarea
                          className="v2-textarea mt-2"
                          value={transcriptionProfile.custom_instructions}
                          onChange={(event) =>
                            updateTranscriptionProfile({
                              custom_instructions: event.target.value,
                            })
                          }
                          placeholder="Instructions that only apply when this Project profile is active…"
                        />
                      </div>
                    </div>
                  )}
                ...
                <section className="v2-setting-section">
                  ...
                  <p className="v2-settings-lead">
                    Only overrides set here differ from Global defaults.
                  </p>
                  {bindingRows.map((row) => (
                    <div className="v2-setting-row" key={row.key}>
                      <div>
                        <strong>{row.label}</strong>
                        <p>
                          {skillBindings[row.key]
                            ? `Project override · ${skillName(skillBindings[row.key])}`
                            : `Inherits Global · ${skillName(row.fallback)}`}
                        </p>
                      </div>
                      <select
                        className="v2-input"
                        aria-label={`${row.label} Skill`}
                        value={skillBindings[row.key] ?? ""}
                        onChange={(event) =>
                          onSkillBindingsChange({
                            ...skillBindings,
                            [row.key]: event.target.value || undefined,
                          })
                        }
                      >
                        ...
                        {skills
                          .filter(
                            (skill) => skill.enabled && row.accepts(skill),
                          )
                          .map((skill) => (
                            <option key={skill.id} value={skill.id}>
                              {skill.name}
              ...
              <div className="v2-list-axis">
                <div className="v2-page-heading">
                  <div className="v2-page-heading-copy">
                    ...
                    <p>
                      Review what this Project may use. Excluding a Source never
                      deletes it from your private Library.
                    </p>
                  </div>
                </div>
                {classificationError && (
                  <div
                    className="rounded-md bg-[#fff4f1] px-3 py-2 text-[14px] text-[#a33d36]"
                    role="alert"
                  >
                    {classificationError}
                  </div>
                ...
                <div className="v2-review-list">
                  {contextGroups.map((group) => {
                    const included = group.items.some((material) =>
                      material.projects.includes(project.name),
                    );
                    const excluded = group.items.some((material) =>
                      material.excludedProjects?.includes(project.name),
                    );
                    const savedOnly = group.items.some((material) =>
                      material.savedOnlyProjects?.includes(project.name),
                    );
                    const representative =
                      group.bundle?.source ?? group.representative;
                    const primary =
                      group.bundle?.primaryComment ?? representative;
                    const suggestedBy = group.items.find((material) =>
                      material.organization?.suggested_projects?.includes(
                        project.name,
                      ),
                    );
                      ...
                      <article className="v2-review-row" key={group.key}>
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            <span className="v2-membership-pill">{state}</span>
                            {suggestedBy?.organization?.confidence !==
                              undefined &&
                            !included &&
                            !excluded ? (
                              <span className="v2-quiet-pill">
                                {Math.round(
                                  suggestedBy.organization.confidence * 100,
                                )}
                                % match
                              </span>
                            ) : null}
                          </div>
                          <h3>{sourceTitle(representative)}</h3>
                          <p>{primary.content}</p>
                          {excluded ? (
                            <div className="v2-library-meta">
                              Your correction prevents automatic re-adding
                            </div>
                          ) : !included ? (
                            <div className="v2-library-meta">
                              {suggestedBy?.organization?.reason ||
                        ...
                        <div className="v2-inline-actions">
                          <Button
                            size="sm"
                            onClick={() => onOpenMaterial(primary.id)}
                          >
                            Open source
                          </Button>
                          {excluded ? (
                            ...
                            <>
                              <Button
                                ...
                                disabled={busy}
                                onClick={() =>
                                  void setProjectMembership(group, "removed")
                                }
                              >
                                Remove from Context
                              </Button>
                              <Button
                                ...
                                disabled={busy}
                                onClick={() =>
                                  void setProjectMembership(group, "excluded")
                                }
                              >
                                Exclude
                              </Button>
                            ...
                            <>
                              <Button
                                ...
                                disabled={busy}
                                onClick={() =>
                                  void setProjectMembership(group, "added")
                                }
                              >
                                Add to Context
                              </Button>
                  ...
                  {!contextGroups.length && (
                    <div className="v2-recovery-card">
                      <p>
                        No Sources are included, suggested, or excluded for this
                        Project yet.
                      </p>
                    </div>
                  )}
            ...
            <>
              <div className="v2-editor-scroll">
                {answerRun ? (
                  <article
                    className="v2-editor-axis"
                    aria-label="Project answer"
                  >
                    <div className="v2-editor-eyebrow">
                      Answer · {answerRun.skill_name}
                    </div>
                    <h1 className="v2-editor-title">{answerRun.instruction}</h1>
                    <div className="v2-editor-body whitespace-pre-wrap">
                      {answerRun.original_output ||
                        answerRun.error ||
                    ...
                    <div className="v2-context-summary">
                      <span>
                        {answerRun.sources.length} frozen Sources · Run{" "}
                        {answerRun.status}
                      </span>
                      <button
                    ...
                    {answerRun.original_output && (
                      ...
                      >
                        ...
                        <Button
                          size="sm"
                          variant="primary"
                          onClick={() => void adoptAnswer("document")}
                        >
                          Save as document
                        </Button>
                        <Button
                          size="sm"
                          onClick={() => setAnswerRun(undefined)}
                        >
                          Back to document
                        </Button>
                      </div>
                  ...
                  >
                    <div className="v2-editor-eyebrow">
                      {previewRevision
                        ? `Revision ${previewRevision.revision} · Read only`
                        : "Document"}
                    </div>
                    <input
                      className="v2-editor-title w-full border-0 bg-transparent outline-none disabled:opacity-60"
                      aria-label="Document title"
                      value={previewRevision?.title ?? documentTitle}
                    ...
                    <textarea
                      ...
                      placeholder={
                        activeDocument
                          ? "Start writing, or ask Logue to draft from this project's sources."
                          : "Create a document, or ask Logue to draft from this project's sources."
                      }
                    />
                    <div className="v2-context-summary">
                      <span>
                        {sourceGroups.length} project sources ·{" "}
                        {visibleSourceIds.length} sources in this revision
                      </span>
                      <div className="v2-inline-actions">
                        <button
                          className="v2-source-excerpt-toggle"
                        ...
                        <button
                          className="v2-source-excerpt-toggle"
                          type="button"
                          disabled={!activeDocument || revisionBusy}
                          onClick={() => void toggleRevisionHistory()}
                        >
                    ...
                    {revisionError && (
                      <div
                        className="mt-3 rounded-md bg-[#fff4f1] px-3 py-2 text-[14px] text-[#a33d36]"
                        role="alert"
                      >
                        {revisionError}
                      </div>
                    ...
                    {historyOpen && (
                      ...
                      >
                        <div className="v2-setting-row !border-0 !px-0 !pt-0">
                          <div>
                            <strong>Revision history</strong>
                            <p>
                              Each revision keeps the Sources used at that time.
                            </p>
                          </div>
                          {previewRevision ? (
                            <Button
                          ...
                          {documentRevisions.map((revision) => (
                            <Button
                              ...
                              variant={
                                ...
                                previewRevision?.revision === revision.revision
                                  ? "primary"
                                  : "secondary"
                              }
                              onClick={() =>
              ...
              {generationError && (
                <div
                  className="mx-auto mb-2 rounded-md bg-[#fff4f1] px-3 py-2 text-[14px] text-[#a33d36]"
                  style={{ width: "min(calc(100% - 48px), 820px)" }}
                  role="alert"
                >
                  {generationError}
                </div>
              ...
              {!projectMaterials.length && (
                <div
                  className="mx-auto mb-2 text-[13px] text-[#73756f]"
                  style={{ width: "min(calc(100% - 48px), 820px)" }}
                >
                  Add Sources to this Project before asking Logue to draft.
                </div>
              )}
              <div className="v2-composer-wrap">
                <ProjectComposer
                  ...
                  disabled={generating || !projectMaterials.length}
                  showVoice={false}
                  mode={requestMode}
                  onModeChange={(nextMode) => {
                    if (nextMode !== "compare") setRequestMode(nextMode);
                  }}
                  placeholder={
                    generating
                      ? `${requestMode === "ask" ? "Answering" : "Drafting"} from project sources…`
                      : requestMode === "ask"
                        ? `Ask ${project.name}`
                        : `Draft with ${project.name}`
                  }
                />
              </div>
            </>
              ...
              <div className="v2-inspector-scroll">
                <div className="v2-source-list">
                  {visibleSourceGroups.map((group, index) => {
                    ...
                    const citedMember = group.items.find((item) =>
                      visibleSourceIds.includes(item.id),
                    );
                    const comment =
                      citedMember && citedMember.id !== web.id
                        ? citedMember
                        : group.bundle?.primaryComment;
                    const focus =
                      citedMember?.id === web.id
                        ? "web"
                        : citedMember
                          ? "comment"
                          : undefined;
                    ...
                    return (
                      <SourceBundleView
                        key={group.key}
                        citation={index + 1}
                        title={sourceTitle(web)}
                        excerpt={web.content}
                        comment={comment?.content ?? "Saved to this Project"}
                        focus={focus}
                        meta={[
                          web.source?.domain || "Logue",
                          citedLabel ? `Cited item · ${citedLabel}` : "",
                        ]
                          .filter(Boolean)
                          .join(" · ")}
                        onSelect={() =>
                          onOpenMaterial(
                            citedMember?.id ?? comment?.id ?? web.id,
                          )
                        }
                        onOpenSnapshot={() =>
                          onOpenMaterial(
                  ...
                  {!visibleSourceGroups.length && (
                    <p className="v2-settings-lead">
                      No Sources are used by this document yet.
                    </p>
                  )}
                </div>
              </div>
