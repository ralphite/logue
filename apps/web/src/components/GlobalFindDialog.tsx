...
export function GlobalFindDialog({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      void Promise.all([
        ...
        .then(([sourceMatches, documentMatches]) => {
  ...
  const results = useMemo<FindResult[]>(() => {
    ...
    const projectResults = projects
      ...
      .map((item) => ({ kind: "project" as const, id: item.name, title: item.name, detail: item.overview || "Project" }));
    return [
      ...projectResults.slice(0, 6),
      ...documentResults.slice(0, 10),
      ...sourceResults.slice(0, 14),
    ];
  }, [documentIds, documents, materials, projects, query, sourceIds]);
    ...
    <div className="v2-global-find-backdrop" role="presentation" onPointerDown={(event) => {
      ...
      <section ref={dialogRef} className="v2-global-find-dialog" role="dialog" aria-modal="true" aria-labelledby="global-find-title" tabIndex={-1}>
        <div className="v2-global-find-input">
          ...
          <input ref={inputRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find anything in Logue" aria-label="Find anything in Logue" />
        ...
        <div className="v2-global-find-results">
          ...
          {results.map((result) => {
            ...
            return <button type="button" key={`${result.kind}:${result.id}`} className="v2-global-find-result" onClick={() => { navigateTo(result); onOpenChange(false); }}>
        ...
        <footer><span>↵ Open</span><span>Esc Close</span></footer>
      </section>
    </div>
  );
}
