  ...
  return (
    ...
    >
      <header className="sticky top-0 z-10 flex h-12 items-center justify-between border-b border-[#eeeeeb] bg-white/95 px-4 backdrop-blur-xl">
        {isPage ? <button type="button" onClick={onClose} className="inline-flex items-center gap-1.5 rounded-md px-2 py-1.5 text-[11.5px] text-[#71726d] hover:bg-[#f1f1ee]"><ArrowLeft size={14} /> Stream</button> : <div className="flex items-center gap-2.5"><span className="text-[11.5px] text-[#777873]">Stream</span><span className="text-[#b7b8b3]">/</span><span className="text-[11.5px] text-[#4f504c]">{materialTitles[material.kind]}</span></div>}
      ...
      <div data-testid="material-detail-reading-column" className={isPage ? "mx-auto w-full max-w-[860px] px-10 pb-10 pt-14 max-[640px]:px-4 max-[640px]:pb-5 max-[640px]:pt-9" : "px-5 pb-8 pt-8 max-[640px]:px-4 max-[640px]:pb-4"}>
          ...
          <section className="mb-7 flex items-start gap-2.5 rounded-md border border-[#e7d5a8] bg-[#fffaf0] px-3 py-2.5" aria-label="Needs review">
            ...
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                <p className="text-[11px] font-semibold text-[#79551f]">Needs review</p>
                {material.organization?.confidence !== undefined && (
                  <span className="text-[9.5px] text-[#a17f49]">Agent confidence {Math.round(material.organization.confidence * 100)}%</span>
                )}
              </div>
              <p className="mt-0.5 text-[10.5px] leading-4 text-[#896b3c]">{material.organization?.reason || "Check whether the project and tags are correct"}</p>
              {hasOrganizationSuggestion && (
                <div className="mt-2">
                  <p className="text-[9.5px] font-medium text-[#9a7640]">Agent suggestion</p>
              ...
              <div className="mt-2 flex flex-wrap gap-1.5">
                {hasOrganizationSuggestion && <button type="button" disabled={organizationSaving} onClick={() => reviewOrganization(true)} className="h-7 rounded-md bg-[#805f2f] px-2.5 text-[10px] font-medium text-white hover:bg-[#6d5028] disabled:opacity-50">Apply suggestion</button>}
                <button type="button" disabled={organizationSaving} onClick={() => reviewOrganization(false)} className="h-7 rounded-md border border-[#e3d2aa] bg-white/60 px-2.5 text-[10px] font-medium text-[#765c34] hover:bg-white disabled:opacity-50">Keep current</button>
              </div>
            </div>
          </section>
        )}
        ...
        {hasAudioChain ? (
          <section aria-label="Voice history">
            <div className="flex items-center justify-between gap-3">
              <h2 className="text-[12px] font-semibold text-[#4f504c]">History</h2>
              <span className="inline-flex items-center gap-1 text-[10px] text-[#858680]"><Check size={11} /> Original record remains unchanged</span>
            </div>
            <ol className="mt-5 ml-3 border-l border-[#dcdcd7]">
              <li className="relative pb-7 pl-6">
                ...
                <div className="flex items-baseline justify-between gap-3"><h3 className="text-[11.5px] font-semibold text-[#4c4d49]">Original audio</h3><span className="text-[9.5px] text-[#9a9b96]">Original</span></div>
                <p className="mt-0.5 text-[10.5px] text-[#92938e]">Saved audio for playback and verification</p>
              ...
              <li className="relative pb-7 pl-6">
                ...
                <div className="flex items-baseline justify-between gap-3"><h3 className="text-[11.5px] font-semibold text-[#4c4d49]">Machine transcript</h3><span className="text-[9.5px] text-[#9a9b96]">Original result</span></div>
                <p className="mt-2 whitespace-pre-wrap text-[12px] leading-5 text-[#747570]">{material.transcript || "No machine transcript was saved"}</p>
              </li>
              <li className="relative pb-7 pl-6">
                ...
                <div className="flex items-baseline justify-between gap-3"><h3 className="text-[11.5px] font-semibold text-[#42453f]">Final text</h3><span className="text-[9.5px] font-medium text-[#638064]">Adopted</span></div>
                <textarea
              ...
              <li className="relative pl-6">
                ...
                <div className="flex items-baseline justify-between gap-3"><h3 className="text-[11.5px] font-semibold text-[#4c4d49]">Follow-up content</h3><span className="text-[9.5px] text-[#9a9b96]">{dependentCount} {dependentCount === 1 ? "item" : "items"}</span></div>
                {dependents.length > 0 ? (
                ...
                ) : <p className="mt-1 text-[10.5px] text-[#9a9b96]">No derived annotations or follow-up content yet</p>}
              </li>
            </ol>
          </section>
        ) : (
          <section>
            <div className="mb-2 flex items-center justify-between">
              <h3 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#858980]">{material.kind === "selection" ? "Original selection" : material.kind === "derived" ? "Derived content" : "Original content"}</h3>
              {!contentChanged && <span className="text-[10px] font-medium text-[#9a9b96]">Editable</span>}
            </div>
            <textarea
              aria-label="Edit material content"
              value={contentDraft}
        ...
        {material.appliedContext && (
          <section className="mt-7 bg-[#fafaf8] px-3.5 py-3">
            <div className="flex items-center justify-between gap-3"><h3 className="text-[11px] font-semibold text-[#656761]">Actual context</h3><span className="text-[9.5px] text-[#999a95]">Recorded at transcription time</span></div>
            ...
            {material.appliedContext.recent_adopted_texts?.length ? <details className="mt-2"><summary className="cursor-pointer text-[10.5px] font-medium text-[#6469c8]">View adopted phrases</summary><ul className="mt-2 space-y-1 border-l-2 border-[#dfe0f4] pl-3 text-[10.5px] leading-4 text-[#73746f]">{material.appliedContext.recent_adopted_texts.map((text, index) => <li key={`${index}-${text.slice(0, 12)}`}>{text}</li>)}</ul></details> : null}
          </section>
        )}

        {material.annotation && (
          <section className="mt-5">
            <h3 className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#858980]">Existing annotations</h3>
            <div className="border-l-2 border-[#cfd1ca] pl-3">
              <div className="mb-1.5 flex items-center gap-2 text-[#777873]">
                <AudioLines size={14} />
                <span className="text-[11px] font-semibold">Annotation</span>
              </div>
        ...
        <section className="mt-5">
          <div className="mb-2 flex items-center justify-between">
            <h3 className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#858980]">Organization</h3>
            {(organizationSaving || material.organization?.status === "pending") && <span className="inline-flex items-center gap-1 text-[10px] text-[#8b8c87]"><LoaderCircle size={11} className="animate-spin motion-reduce:animate-none" /> {organizationSaving ? "Saving" : "Organizing automatically"}</span>}
          </div>
          <div>
            <p className="mb-1.5 text-[10.5px] text-[#8b8c87]">Projects</p>
            <div className="flex flex-wrap gap-1.5">
              ...
              {projectOptions.length === 0 && <span className="text-[10.5px] text-[#a0a19c]">No projects yet</span>}
            </div>
          </div>
          <div className="mt-3">
            <p className="mb-1.5 text-[10.5px] text-[#8b8c87]">Tags</p>
            <div className="flex flex-wrap gap-1.5">
              {tagsDraft.map((tag) => <span key={tag} className="inline-flex h-7 items-center gap-1 rounded-md border border-[#e0e1dc] px-2.5 text-[10.5px] text-[#74786f]">#{tag}<button type="button" disabled={organizationSaving} onClick={() => removeTag(tag)} className="text-[#a0a19c] hover:text-[#555651]" aria-label={`Remove tag ${tag}`}><X size={10} /></button></span>)}
              <input value={newTag} disabled={organizationSaving} onChange={(event) => setNewTag(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === ",") { event.preventDefault(); addTag(); } }} onBlur={addTag} className="h-7 min-w-28 flex-1 rounded-md border border-[#deded9] px-2.5 text-[10.5px] outline-none placeholder:text-[#a8a9a4] focus:border-[#aaa]" placeholder="Add a tag, then press Enter" aria-label="Add tag" />
            </div>
          </div>
        ...
        {parents.length > 0 ? (
          <section className="mt-7">
            <div className="mb-2 flex items-center gap-2 text-[11px] font-semibold text-[#70746b]"><Sparkles size={13} className="text-[#6b72de]" /> Derived from {parents.length} {parents.length === 1 ? "item" : "items"}</div>
      ...
      <footer className="mt-10 border-t border-[#e7e7e2] bg-[#fcfcfa] py-4 max-[640px]:mt-7 max-[640px]:py-2">
        ...
        {deleteConfirming ? (
          <div className="mt-2 rounded-md border border-[#efd3ce] bg-[#fff8f6] p-3">
            <p className="text-[10.5px] leading-4 text-[#8e4a43]">Delete this {materialTitles[material.kind].toLowerCase()}? Its original audio will also be deleted when nothing else references it. {dependentCount > 0 ? `${dependentCount} derived ${dependentCount === 1 ? "item will" : "items will"} remain.` : ""}</p>
            {deleteError && <p className="mt-1 text-[10px] text-[#b0443a]">{deleteError}</p>}
            <div className="mt-2 flex justify-end gap-1.5"><button type="button" disabled={removing} onClick={() => { setDeleteConfirming(false); setDeleteError(undefined); }} className="h-7 rounded px-2.5 text-[10.5px] text-[#6d6e69] hover:bg-white">Cancel</button><button type="button" disabled={removing} onClick={() => void remove()} className="inline-flex h-7 items-center gap-1.5 rounded bg-[#b2483f] px-2.5 text-[10.5px] font-medium text-white disabled:bg-[#cf9a95]">{removing && <LoaderCircle size={11} className="animate-spin motion-reduce:animate-none" />}{removing ? "Deleting…" : "Confirm delete"}</button></div>
          </div>
        ) : (
          <button type="button" onClick={() => setDeleteConfirming(true)} className="mt-1 inline-flex h-8 w-full items-center justify-center gap-1.5 rounded-md text-[10.5px] font-medium text-[#a54b42] hover:bg-[#f9ece9] max-[640px]:h-11"><Trash2 size={12} /> Delete this {materialTitles[material.kind].toLowerCase()}{dependentCount > 0 ? ` · ${dependentCount} derived ${dependentCount === 1 ? "relationship" : "relationships"}` : ""}</button>
        )}
      </footer>
      </div>
    </Root>
