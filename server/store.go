...
func (s *Store) migrateLegacyUncertainOrganizations() error {
	...
	entries, err := os.ReadDir(filepath.Join(s.root, "items"))
	if err != nil {
		return fmt.Errorf("read items for organization migration: %w", err)
	}
	for _, entry := range entries {
		if entry.IsDir() || filepath.Ext(entry.Name()) != ".json" {
			continue
		...
		data, readErr := os.ReadFile(filepath.Join(s.root, "items", entry.Name()))
		if readErr != nil {
			return fmt.Errorf("read item %s for organization migration: %w", entry.Name(), readErr)
		}
		var item Material
		if unmarshalErr := json.Unmarshal(data, &item); unmarshalErr != nil {
			return fmt.Errorf("decode item %s for organization migration: %w", entry.Name(), unmarshalErr)
		}
		organization := item.Organization
		if organization == nil || organization.Status != "needs_review" || organization.Confidence <= 0 || organization.Confidence >= organizationConfidenceThreshold {
			continue
		}
		legacyAssignments := len(organization.SuggestedProjects) == 0 && len(organization.SuggestedTags) == 0 && (len(item.Projects) > 0 || len(item.Tags) > 0)
		nilArrays := item.Projects == nil || item.Tags == nil
		if !legacyAssignments && !nilArrays {
			continue
		}
		if legacyAssignments {
			organization.SuggestedProjects = normalizeStrings(item.Projects)
			organization.SuggestedTags = normalizeStrings(item.Tags)
		...
		if writeErr := s.writeItemLocked(item); writeErr != nil {
			return fmt.Errorf("migrate uncertain organization for %s: %w", item.ID, writeErr)
		}
	}
	return nil
}
