# Logue V1 历史产品目标与完成契约

> **V1 历史文档。** 现有已发布产品统一视为 V1。本文件保留 V1 的实现与验收历史，但不再是 V2 产品设计依据。当前 V2 唯一权威设计见 [`docs/design/logue-ai-product-positioning-2026-08-04.md`](./docs/design/logue-ai-product-positioning-2026-08-04.md)；当前只迭代设计，不开始实现。

状态：**V1 已发布；V2 设计中。**

以下内容是 V1 阶段的历史目标。它用于解释已发布实现、测试和遗留验收记录，不得自动成为 V2 的功能、IA 或交付范围。

解释 V1 历史时，如本文件与更早的规格、代码、数据、测试、截图或模型方案冲突，以用户在 V1 阶段的最新明确纠正和本文件为准；V2 不适用这条规则。
...
**桌面 Linux → MacBook 的 LAN/远程服务连接是当前最高优先级，不再延期。iPhone 与移动端不在当前产品支持范围。** 现有响应式体验不应被故意破坏，但不投入移动端专项优化或真机验收。
