...
### DR-020 — 现有实现与数据不构成产品约束

- **优先级：** 产品与架构基础
- **状态：** 已按用户最新决定生效
- **决策：** 现有代码、UI、信息架构、对象名、schema、routes、默认值和本地数据都是可删除的原型输入，不享有保留优先权。产品从最新用户结果和 clean-slate 规格出发；复用旧实现只因它恰好符合新模型，而不是为了降低改动量。
- **用户可见影响：** `Stream / Documents / Skills / Material / Tags / 自动归档` 等当前表面或对象可以被删除或重命名；发布新模型时，现有测试数据可能无法继续读取。
- **数据处理：** 破坏性切换前在可行时创建备份并尝试一次 best-effort 导入；迁移失败不阻止发布。切换后删除迁移代码、旧 schema、旧 routes、fallback、aliases 和双版本 UI。
- **替代方案：** 以当前实现和数据为真相渐进扩展。它降低短期工程成本，但会让原型结构固化为产品边界，并持续增加兼容债务。
- **已有证据：** 用户于 2026-08-04 明确表示现有产品、代码和设计不应形成 legacy 约束，已有内容都可以更新或删除；随后进一步明确现有数据也无需保证保留，能迁移最好但不是要求。2026-08-04 已将原运行数据完整复制到 `/Users/yadong/dev2/logue-data-backups/clean-slate-20260804-213000`，再可恢复地移至 `clean-slate-retired-20260804-214500`；新的真实 Google 选区保存后生成 `src_6a3e4870a00ebca1` 并在服务重启后的 Library 可见。
- **需要用户决策：** 无；这是用户已明确改变的项目规则。
...
- **优先级：** P1
- **状态：** 历史实现证据；clean-slate Search 以 DR-020 与 v2 规格重新实现，不得保留旧 routes/schema
...
### DR-015 — 旧五导航候选收敛方案（已由 DR-020 取代）

- **优先级：** 产品方向
- **状态：** 已废止；不得作为当前 IA、对象或实现依据
- **取代说明：** 用户随后明确要求现有产品、代码、设计和数据均不构成 legacy 约束。当前三对象、三入口与 `Capture → Recall → Make` 方案以 DR-020 和 clean-slate v2 为准；下列内容只保留为历史记录。
