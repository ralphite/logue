...
- **实现与静态证据：** Host 以单一 root transaction 归档完整 recovery revision、写入活动列表不可见的 tombstone并标记原 adoption undone；同 ID 重试不改文件，revision 已变化返回 409 且数据 byte-for-byte 不变，重新保存使用新 ID/新 Document并继承 frozen Sources。Extension API 区分 active Document/tombstone并传播 HTTP status；普通页面、Google Docs proxy 与 Side Panel 均保存 typed adoption、保留 Candidate并按 action 显示/执行 Undo。Web Project Draft 与 Library Recover Candidate 也复用同一 typed adoption/Undo outcome，均不再因新建 Document 清空 Candidate；409 Conflict 清除死 Undo并恢复 Save 菜单。Web Document Action Replace 的 409 同样清除失效 Undo并恢复 Apply，网络失败保留原 adoption 供重试；Side Panel 与 Selection/Page Candidate 的终态 4xx 不再提供无效 Retry，网络/503 则保留同一 adoption并显示 Retry，Google Docs proxy 复用相同状态。两条窄 Host 合同测试、Google Docs bridge 与 Web/Extension adoption state 测试、Python compile、Web/Extension typecheck 与 diff check 通过；未运行 Browser/CU。
