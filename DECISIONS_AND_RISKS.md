...
- **实现与静态证据：** Host 以单一 root transaction 归档完整 recovery revision、写入活动列表不可见的 tombstone并标记原 adoption undone；同 ID 重试不改文件，revision 已变化返回 409 且数据 byte-for-byte 不变，重新保存使用新 ID/新 Document并继承 frozen Sources。Extension API 区分 active Document/tombstone并传播 HTTP status；普通页面、Google Docs proxy 与 Side Panel 均保存 typed adoption、保留 Candidate并按 action 显示/执行 Undo。两条窄 Host 合同测试、Google Docs bridge 测试、Python compile、Web/Extension typecheck 与 diff check 通过；未运行 Browser/CU。
