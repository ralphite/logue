...
- Product authority order is: the user's latest explicit decision, the current product definition, the accepted UI/UX, then reusable invisible engineering primitives. Existing code, tests, reviewers, and implementation convenience may not override that order.
...
- The complete confirmed feature scope remains required. Core focus sets implementation order; it never authorizes deleting, refusing, or indefinitely postponing a confirmed feature. Do not add an unconfirmed product direction merely because it is technically possible or an old implementation exists.
