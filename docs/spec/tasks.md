...
| **X28** | 边栏预览卡崩掉整个页面:`preview` 读了 undefined 的 `.length` | 你贴的报错:`Uncaught TypeError: Cannot read properties of undefined (reading 'length') at Object.preview (index-D-gr9amm.js:15:28633)`。`preview` 是边栏行的预览卡([RailList.tsx](web/src/app/RailList.tsx) 里 `entry.preview()`,数据由各个 Route 提供)。有一条 entry 的预览函数拿到了 `undefined` 还去读 `.length` —— 多半是某个数组字段(Sources、tags、versions 之类)在这条记录上根本不存在,而预览没做防备。**要做的**:①从压缩栈回到源码,定位是哪个 Route 的 `preview`;②那个字段为什么会缺,是后端没给还是这条记录本来就没有;③**顺手把所有 `preview` 都过一遍** —— 同一类写法通常不止一处。**这条要紧的地方在于它不是显示错了,是整页崩**:一个预览卡不该有能力弄垮页面,鼠标划过一行就白屏。栈里那个文件名 `index-D-gr9amm.js` 和 X27 是同一个构建 |
