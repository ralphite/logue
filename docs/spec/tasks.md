...
| **F7** | 接一个免费模型 API,替身退场 | 你的原话是 "find free api that you can use for llm gen and transcription"。查完:**首选 Groq** —— 免费档不用信用卡,生成(Llama 3.x,约 14,400 请求/天)和**转写(Whisper,2,000 次/天)一家全包**,OpenAI 兼容接口;**备选:重新申请一个 Gemini key**(免费档 250–1,500 请求/天,接口都是现成的,零代码)。要做的:①你在 console.groq.com 建号拿 key(账号和 key 我不能替你建);②若走 Groq,Host 加一个 OpenAI 兼容 provider(现在只有 Gemini 形状);③key 一到,S3 全量重验 |
