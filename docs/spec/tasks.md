...
| **X28** | 选区答案出现十来秒后,和工具条一起整个消失 —— **观察到了,原因没坐实** | 量到的:在 Logue 自己的长文档上跑一个选区 Skill,答案就地出现(引用角标、`1 Sources`、Insert),**十一秒里都在**;第十二秒,选区塌了、`data-logue-surface` 变成 `none`、Insert 没了 —— 三样**同时**归零。同时归零不像"选区没了所以工具条收起来",像**内容脚本被整个换掉**(换代/重挂会把状态清空)。两次复现都因别的原因中断(一次选区没建起来、一次按钮没找到),所以**原因我没坐实,不写结论**。为什么值得查:如果是换代造成的,那么"一次自更新会吃掉一个还没被采纳的答案" —— 和录音受保护是同一类问题(X10/X27 的规矩是"别打断人还能用的东西"),而答案比录音更容易在几秒内被吃掉。先查清是谁清的,再谈要不要让它延后 |
