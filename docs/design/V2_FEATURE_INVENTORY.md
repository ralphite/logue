...
- V2-CMD-06 — INTEGRATED — Side Panel Ask/Draft Candidate 编辑与 Insert/Copy/Keep/Document 使用统一 adoption 合同；Keep 以稳定 identity 物化永久 AI Source、继承 exact Run Sources 并即时 Undo；Document 明确选择新建或更新当前 Project Document，更新创建新 revision并保留即时 Undo；Undo 的终态 409 清除失效 adoption且不显示 Retry，网络/503 保留同一 adoption 与 Retry。
...
- V2-CAP-08 — INTEGRATED — editable selection Replace/Undo、静态 Copy/Keep 与可选 Document target 使用统一 adoption 合同；Document 更新创建新 revision并可从 Candidate 撤销为新的恢复 revision，新建 Document 后保留 Candidate 并可即时 Undo 为可恢复 tombstone；Document Undo 的终态 4xx 清除失效 adoption并恢复正常操作，只有网络/503 保留同一 adoption 与 Retry；普通页面、Google Docs proxy 与 Side Panel 使用同一 typed adoption。
...
- V2-DOC-05 — INTEGRATED — Action Replace/Copy/Keep 使用统一 AI Source / Document revision adoption 合同；Keep 成功后保留 Candidate，并以原 adoption ID 提供即时 Undo；Replace Undo 的终态 4xx 清除失效入口并恢复 Apply，409 显示 changed，只有网络/503 保留同一 adoption ID 与 Retry。
