...
| P0 | Google Docs 行内语音（F25） | READY_FOR_REAL_ENV | 当前 unpacked Extension 已在真实 Docs canvas 进入 Start → Cancel/Stop → Transcribing；自动化采集没有人声，Gemini 因此没有返回文字。仍缺真实口述后的单次保存与单次插入证明；启动器键盘可达性也是独立 P1。 | 用真实口述短句完成保存一次、插入一次、不触发 Docs 命令；Esc 取消保持零写入。 |
