...
|---|---|---|---|---|
| P0 | Linux / LAN 远程服务（F22） | IN_PROGRESS | 打开 Logue Web App 后的明确 host 一键授权已在真实 Mac Chrome + 非回环地址通过；仍缺安装器的 `0.0.0.0` / `127.0.0.1` 交互选择并默认前者，以及真实 Linux systemd user service、防火墙分配且可能变化的域名、跨机保存/读取、Chrome/MV3/Linux 重启恢复闭环。 | GitHub Actions 证明 Linux 非回环监听；安装时可选择监听地址且默认 wildcard，并有无公网认证警告；在真实 Linux + Mac Chrome 完成连接、地址替换、保存/读取、浏览器重启和服务重启。 |
