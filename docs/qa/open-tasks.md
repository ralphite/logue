...
| P1 | Selection Skills 文本与菜单稳定性 | IN_PROGRESS | Document 已在真实 Chrome 确认多行写回、自动保存期间首次应用和持久化；取消菜单的 pending refresh 竞态也已修复。仍缺当前 Release 在真实 Extension textarea/contenteditable 的多行与取消后迟到重现回归。 | textarea、contenteditable、Document 均保留换行；菜单只跟随当前有效选区，取消选择/失焦/导航/漂移后立即关闭且不被迟到事件唤回。 |
...
| P1 | Release 跨机验收（F13） | READY_FOR_REAL_ENV | `v0.2.8` 的公开校验和、独立 Extension、服务首装/覆盖和数据保留已通过隔离验证；仍缺另一台真实 Linux/Mac 的服务安装、Chrome Load unpacked/Reload、数据保留和覆盖升级。 | 从公开 Release 在另一台机器完成首次安装和覆盖升级，真实数据不被覆盖，失败可回滚。 |
