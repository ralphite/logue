...
| B06 | 自动 Tag / 项目关联机械且不相关 | PASS | 由可定制 Automatic organization Agent 做内容分类；高置信安静写入，低置信只给建议并显示英文理由、置信度和人工复核；无可靠匹配时允许空关联。 |
...
| B12 | Documents / Agents selected 背景范围不一致 | PASS | 两行共用相同 44px row skeleton，selected 背景覆盖整行并包含右侧 plus；真实 Chrome 证据：`/tmp/logue-generate-documents-selected.png`、`/tmp/logue-generate-agents-selected.png`。 |
...
| B21 | Web UI / Web code 混用中文 | PASS | 当前 `apps/web/src` 产品文案、无障碍文案和测试命名无中文；用户自有资料、项目名和自建 Agent 原样保留。系统 Agent 和固定分类理由已安全迁移为英文。 |
...
| F08 | Logue Web App 自己也能使用 Extension | PARTIAL | 已移除全页禁用，真实 Chrome 的 Logue Task 输入框出现 1 个语音和 1 个 Agent 入口；麦克风可开始录音、Esc 可取消且无重复注入。仍需在重载最新 Extension 后补一次“停止 → Gemini → 保存 → 插入”完整证据。 |
...
1. **P1**：在真实 Chrome 重载最新 Extension，完成 Logue Web App 内“停止 → Gemini → 保存 → 插入”全链路，确认只注入一组入口。
2. **P1（外部设备）**：物理 iPhone 访问局域网入口，完成触控、旋转、刷新、Stream / Projects / Generate / 文档编辑。
3. **P1（发布）**：当前主线稳定后发布下一版本，并用同一条 curl 命令从 `v0.1.1` 覆盖升级，确认数据和启动项保持。
